...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($email)) {
        ...
        $sql = "SELECT id FROM customers WHERE email = ? OR name = ?";
        $stmt = $conn->prepare($sql);
        ...
        if ($stmt->num_rows > 0) {
            ...
            if ($stmt->execute()) {
                $success = "Customer created successfully. Please set a password.";
                logUserActivity($_SESSION['user_id'], $_SESSION['role'], 'Create customer');
                header("Location:manage_customers.php");


            } else {
                $err = "Error creating Customer: " . $stmt->error;
            }
        }
        $stmt->close();
    }
...
<div class="admin_container">
    <h3 class="form-heading">Create Customer</h3>
