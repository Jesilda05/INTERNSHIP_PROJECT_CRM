...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($email)) {
        ...
        $sql = "SELECT id FROM salesmanagers WHERE email = ? OR name = ?";
        $stmt = $conn->prepare($sql);
        ...
        if ($stmt->num_rows > 0) {
            $err = "Name or email already exists.";
        } else {
            $sql = "INSERT INTO salesmanagers (name, email) VALUES (?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('ss', $name, $email);

            if ($stmt->execute()) {
                $success = "Sales Manager created successfully. Please set a password.";
                logUserActivity($_SESSION['user_id'], $_SESSION['role'], 'Create salesmanager');
                header("Location:manage_sales_manager.php");


            } else {
                $err= "Error creating Sales Manager: " . $stmt->error;
            }
        }
        $stmt->close();
    }
...
<div class="admin_container">
    <h3 class="form-heading">Create Sales Manager</h3>
    <?php if (!empty($err)): ?>
        <div class="error"><?php echo $err; ?></div>
    <?php endif; ?>

<form action="create_sales_manager.php" method="POST">
    Name: <input type="text" name="name" required><br>
    Email: <input type="email" name="email" required><br>
    <button type="submit">Create Sales Manager</button>
</form>
